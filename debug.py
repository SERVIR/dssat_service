...
def ingest_latest_forecast():
    ...
    # Forecast map
    file = "/home/dquintero/dssat_service/forecast_data/Kenya/latest_forecast.geojson"
    db.add_latest_forecast(con, schema, file)
    # All simulations results
    results_df = pd.read_csv(
        "/home/dquintero/dssat_service/forecast_data/Kenya/forecast_20241107.csv"
    )
    db.dataframe_to_table(
        f"postgresql+psycopg2://{con.info.user}:eQY3_Fwd@localhost:{con.info.port}/{con.info.dbname}",
        results_df,
    ...
    overview_df = pd.read_csv(
        "/home/dquintero/dssat_service/forecast_data/Kenya/forecast_overview_20241107.csv"
    )
    db.dataframe_to_table(
        f"postgresql+psycopg2://{con.info.user}:eQY3_Fwd@localhost:{con.info.port}/{con.info.dbname}",
        overview_df,
...
def ingest_cultivars():
    ...
    ing.ingest_cultivars(
        con, 
        schema, 
        "/home/dquintero/dssat_service/cultivar_selection/Kenya/cultivar_table.csv"
    )
    con.close()
