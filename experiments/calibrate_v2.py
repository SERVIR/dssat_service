...
def process_cultivar(admin1, cultivar, inputs_dict):
    ...
    for month in PLANT_MONTHS:
        ...
        run_df.to_csv(
            f"/home/user/dssat_service/regional_service/experiments/parameters/{COUNTRY.lower()}/all_runs/{admin1}_{cultivar}_{month}.csv", index=False
        )
        if len(obs_yield) > 3:
    ...
    ).to_csv(f"/home/user/dssat_service/regional_service/experiments/parameters/{COUNTRY.lower()}/{admin1}_{cultivar}.csv", index=False)
